...
export async function updateUserProfile(userId: string, updates: Partial<User>) {
  ...
  if (currentCache) {
    ...
    const updatedData = { ...data, ...updates, updated_at: new Date().toISOString() };
    localStorage.setItem(cacheKey, JSON.stringify({
      data: updatedData,
      timestamp: Date.now()
    }));
...
// OAuth operations with caching
export async function getOAuthConnections(userId: string) {
  const cacheKey = `${CACHE_PREFIX}oauth-${userId}`;
